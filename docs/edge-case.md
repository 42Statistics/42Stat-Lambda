...
- scale team 없이 과제를 통과하게 되면 (exam 제외) NaN 오류가 생기는데, 이는 인위적인 데이터 삽입으로 인해 생기는 경우라고 볼 수 있다.
  - internship 최종 과제의 경우 scale team 없이 과제를 통과한다. 현재는 이로인해 scale team 이 없으면 team 의 final mark 를 그대로 반영한다.
...
- 2023년 06월 18일 경, intra 에 장애가 생겼던 것으로 추정. 해당 시점에 생긴 잘못된 location 들이 여럿 있고, 이는 intra 서버와 stat db 에서 현재 삭제된 상태이다.
- primary 가 false 인 경우 intra 에서 데이터를 갖고 있지만 집계하지 않는 것으로 보인다...
- 종종 end_at 이 제대로 갱신되지 않는 오류가 있음. 현재 이런 경우를 대비해 따로 요청을 보내보는 중
